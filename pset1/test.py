...
class TestInvertida(unittest.TestCase):
    ...
    def test_invertida_2(self):
        ...
        # imagem 4 x 1
        imagem = pset1.Imagem(4, 1, [29, 89, 136, 200])
        resultado = imagem.invertida()
        esperado = pset1.Imagem(4, 1, [226, 166, 119, 55])
        self.assertTrue(resultado, esperado)

    # teste para a imagem invertida do peixe.
    def test_invertida_bluegill(self):
        # Carregar a imagem bluegill.png
        imagem_original = pset1.Imagem.carregar('test_images/bluegill.png')

        # Aplicar o filtro de inversão
        resultado = imagem_original.invertida()

        # Criar a imagem esperada invertendo manualmente os pixels
        esperado = pset1.Imagem(imagem_original.largura, imagem_original.altura,
                                [255 - pixel for pixel in imagem_original.pixels])

        # Verificar se a imagem invertida está correta
        self.assertEqual(resultado.pixels, esperado.pixels)

    # Teste para a função de correlação
    def test_correlacao(self):
        # Criar a imagem de entrada com os valores fornecidos
        largura = 3
        altura = 3
        pixels = [80, 53, 99, 129, 127, 148, 175, 174, 193]
        img = pset1.Imagem(largura, altura, pixels)
        ...
        # Calcular o resultado da correlação
        resultado_correlacao = img.correlacao(kn)

        # Valor esperado
        valor_esperado = 32.76  # Valor esperado calculado manualmente

        # Verificar se o valor do pixel destacado está próximo do valor esperado
        self.assertAlmostEqual(resultado_correlacao.get_pixel(
            1, 1), valor_esperado, delta=0.01)

    # Teste para a imagem do porco
